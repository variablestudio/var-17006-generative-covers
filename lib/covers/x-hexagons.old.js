...
function drawShape (x, y, solid, chaos) {
  if (chaos == true) { var counter = Math.round(Math.random() * ((numberOfSides / 2) - 1)) } else {
    var counter = 0
  }
...
function makeCover (book) {
  ...
  title = typo.addLigatures(title, 'Andada')
    crayon.font('Andada', titleFontSize, '', 0).fill('#000000').paragraph('left', 0.25, titleWidth, true).text(title);
