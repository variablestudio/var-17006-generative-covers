...
function makeCover (book) {
  ...
  crayon.font('BenchNineLight', titleFontSize, 'normal').fill('#000000')
    .paragraph('left', -0.2, titleWidth, true).text(title)
  ...
  var tmp = crayon.context.measureText(author.name[0] + '').width
  ...
  // crayon.context.fillText(author.name[0], quarter_x, quarter_width / 2)
  // crayon.translate(tmp, 0);
  // crayon.font("UbuntuCondensed", authorFontSize, author.name[1]).fill("#313131").paragraph("left", -0.25, titleWidth, true).text(author.name[0], 0, authorFontSize/2);
