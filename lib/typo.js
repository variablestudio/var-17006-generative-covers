...
function drawTextInRect ({ r, font, text, bounds, debug, fontSize, lineHeight, padding, textAlign, verticalAlign, textColor }) {
  ...
  while (fitFontSize > 1) {
    ...
    fitDescender = fitFontSize * font.descender / font.unitsPerEm
  ...
  lines.forEach((line, i) => {
    ...
    if (verticalAlign === 'top') {
      ...
      ty = y + height + fitLineHeight * (i + paddingTop) + fitAscender - lines.length * fitLineHeight
      textBounds[1] = Math.min(ty - fitAscender, textBounds[1])
    } else {
...
function textPath ({ r, font, text, x, y, fontSize, options }) {
  const commands = font.getPath(text, 0, 0, fontSize, options).commands
  const path = r.path(x, y)
  for (let i = 0; i < commands.length; i++) {
    ...
    if (cmd.type === 'M') {
      ...
      path.curveTo(cmd.x1, cmd.y1, cmd.x, cmd.y)
    } else if (cmd.type === 'Q') {
