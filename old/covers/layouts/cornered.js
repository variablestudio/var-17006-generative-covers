...
function makeCover (book, pattern) {
  ...
  pattern = (pattern !== undefined) ? pattern : Math.round(Math.random() * 5)
  //pattern = -1
  switch (pattern) {
        // Pattern selector
    case -1:
      patterns.DividedLetters(crayon, 0, 0, crayon.canvas.width, crayon.canvas.height, 32, 128, style)
      break;
    case 0:
  ...
  var titleFontSize = crayon.canvas.height * 0.048
  ...
  crayon.context.fill()
    ...
    // crayon.style('author').text(author.name + " " + author.surname, titleX, crayon.canvas.height - 40);
