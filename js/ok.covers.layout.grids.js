...
function makeCover (book) {
  ...
  for (r = 1; r < rows - 1; r++) {
    ...
    var new_y = 25 + Math.round((750 / rows) * Math.round(utils.remap(random_n, 0, 1000000, 1, rows)))
    rowsArray[r - 1] = new_y
  ...
  for (rr = 0; rr < rowsArray.length; rr++) {
    for (c = 1; c < cols - 1; c++) {
      var random_n = Math.random() * 1000000
      var new_x = 25 + Math.round((550 / cols) * Math.round(utils.remap(random_n, 0, 1000000, 1, cols)))
      colsArray[c - 1] = new_x
